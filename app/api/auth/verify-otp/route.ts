...
export async function POST(request: Request) {
  try {
    ...
    if (!stateId || !lgaId || !otp) {
      return NextResponse.json({ error: "State, LGA, and OTP are required" }, { status: 400 })
    }
    ...
    const otpSession = await db.collection<OTPSession>("otp_sessions").findOne({
      lgaId: uniqueLgaId,
    ...
    if (otpSession.otp !== otp) {
      // Increment attempts
      await db.collection<OTPSession>("otp_sessions").updateOne({ _id: otpSession._id }, { $inc: { attempts: 1 } })
    ...
    if (!lgaRecord) {
      return NextResponse.json({ error: "LGA not found or inactive" }, { status: 404 })
    }
    ...
    // Delete OTP session
    await db.collection<OTPSession>("otp_sessions").deleteOne({ _id: otpSession._id })

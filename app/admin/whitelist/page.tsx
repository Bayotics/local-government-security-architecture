    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">LGA Whitelist Management</h1>
            <p className="text-muted-foreground">Manage LGA access and device bindings</p>
          </div>
        </div>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            {isLoading ? (
              ...
              lgas.map((lga) => (
                <div key={lga._id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold">{lga.lgaName}</h3>
                      <Badge variant={lga.status === "active" ? "default" : "secondary"}>{lga.status}</Badge>
                      {lga.boundDeviceId && <Badge variant="outline">Device Bound</Badge>}
                    </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    {lga.boundDeviceId && (
                      <Button variant="outline" size="sm" onClick={() => handleResetDevice(lga.lgaId)}>
